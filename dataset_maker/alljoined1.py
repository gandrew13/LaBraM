...
class Alljoined1(Dataset):
    ...
    def __getitem__(self, index):
        ...
        # data = self.filter_channels(data, ["O1", "OZ", "O2", "PO7", "PO3", "POZ", "PO4", "PO8"]) # keep only the occipital lobe channels, since they are related to the visual cortex
        data = self.filter_channels(data, ["O1", "OZ", "O2", "PO7", "PO3", "POZ", "PO4", "PO8", "P7", "P5", "P3", "P1", "PZ", "P2", "P4", "P6", "P8"]) # keep only the occipital lobe channels, since they are related to the visual cortex
        #data = self.filter_channels(data, ["O1", "OZ", "O2", "PO7", "PO3", "POZ", "PO4", "PO8", "P7", "P5", "P3", "P1", "PZ", "P2", "P4", "P6", "P8", "TP7", "CP5", "CP3", "CP1", "CPZ", "CP2", "CP4", "CP6", "TP8"]) # keep only the occipital and central lobe channels, since they are related to the visual cortex
        #print(len(data), len(data[0]))

        return torch.tensor(data) #self.preprocess_sample(data, seq_len=self.num_chunks, labels=self.labels[image_id])
    ...
    def filter_channels(self, data: list, channels_to_keep: list) -> list:
        ...
        return [data[idx][:300] for idx, ch in enumerate(alljoined1_ch_order) if ch in channels_to_keep]
...
def load_alljoined1_dataset(config: Dict, time_window: int, stride_size: int) -> tuple[Dataset, Dataset, Dataset]:
    ...
    train_data = Alljoined1(ds['train'], labels, sample_keys=['inputs', 'attention_mask'])
    test_data = Alljoined1(ds['test'], labels, sample_keys=['inputs'])
