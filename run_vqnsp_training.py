...
import utils
from dataset_maker.alljoined1 import load_alljoined1_dataset
...
def main(args):
    ...
    train_ds, _, test_ds = load_alljoined1_dataset(args, time_window, stride_size=200)
    ...
    data_loader_train = torch.utils.data.DataLoader(train_ds, sampler=sampler_train, batch_size=args.batch_size, num_workers=args.num_workers, pin_memory=args.pin_mem, drop_last=True)
    data_loader_val = torch.utils.data.DataLoader(test_ds, sampler=sampler_val, batch_size=int(1.5 * args.batch_size), num_workers=args.num_workers, pin_memory=args.pin_mem, drop_last=False)

    num_training_steps_per_epoch = len(train_ds) // args.batch_size // 1
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        train_stats = train_one_epoch(
            model, [data_loader_train,], optimizer, device, epoch, loss_scaler, args.clip_grad, log_writer=log_writer, start_steps=epoch * num_training_steps_per_epoch, lr_schedule_values=lr_schedule_values,
            ch_names_list=[["O1", "OZ", "O2", "PO7", "PO3", "POZ", "PO4", "PO8"],], args=args)
        ...
        if args.output_dir:
            ...
            test_stats = evaluate([data_loader_val], model, device, log_writer, epoch, ch_names_list=val_ch_names_list, args=args)
            print(f"Validation loss of the network on the {sum([len(dataset) for dataset in [data_loader_val]])} test EEG: {test_stats['loss']:.4f}")
